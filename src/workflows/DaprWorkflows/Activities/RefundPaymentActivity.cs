...
public class RefundPaymentActivity : WorkflowActivity<PaymentRequest, object?>
{
    ...
    {
        _logger.LogInformation("Refunding payment: {RequestId} for ${totalCost}", req.RequestId, req.TotalCost);

        // Simulate slow processing
        await Task.Delay(TimeSpan.FromSeconds(5));

        _logger.LogInformation("Payment for request ID '{RequestId}' refunded successfully", req.RequestId);
