...
class UpdateInventoryActivity : WorkflowActivity<CustomerOrder, object?>
{
    ...
    public override async Task<object?> RunAsync(WorkflowActivityContext context, CustomerOrder req)
    {
        ...
        await _client.SaveStateAsync(storeName, req.OrderItem.Name.ToLowerInvariant(),new InventoryItem(ProductId: product.ProductId, 
                                                                                                        Name: req.OrderItem.Name, 
                                                                                                        PerItemCost: product.PerItemCost, 
